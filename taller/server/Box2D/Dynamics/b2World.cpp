/*
 k* Copyright (c) 2006-2011 Erin Catto http://www.box2d.org
 *
...
void b2World::Solve(const b2TimeStep& step) {
	...
	b2Island island(m_bodyCount, m_contactManager.m_contactCount, m_jointCount,
...
void b2World::SolveTOI(const b2TimeStep& step) {
	b2Island island(2 * b2_maxTOIContacts, b2_maxTOIContacts, 0,
			&m_stackAllocator, m_contactManager.m_contactListener);
	...
	for (;;) {
		...
		for (int32 i = 0; i < 2; ++i) {
			...
			if (body->m_type == b2_dynamicBody) {
				for (b2ContactEdge* ce = body->m_contactList; ce; ce =
						ce->next) {
...
void b2World::Step(float32 dt, int32 velocityIterations,
...
struct b2WorldQueryWrapper {
	bool QueryCallback(int32 proxyId) {
		b2FixtureProxy* proxy = (b2FixtureProxy*) broadPhase->GetUserData(
				proxyId);
...
void b2World::DrawShape(b2Fixture* fixture, const b2Transform& xf,
		const b2Color& color) {
...
void b2World::DrawDebugData() {
	...
	if (flags & b2Draw::e_pairBit) {
		...
		for (b2Contact* c = m_contactManager.m_contactList; c; c =
				c->GetNext()) {
