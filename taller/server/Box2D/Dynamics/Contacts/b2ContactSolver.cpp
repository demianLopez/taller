...
b2ContactSolver::b2ContactSolver(b2ContactSolverDef* def) {
	...
	m_positionConstraints =
			(b2ContactPositionConstraint*) m_allocator->Allocate(
					m_count * sizeof(b2ContactPositionConstraint));
	m_velocityConstraints =
			(b2ContactVelocityConstraint*) m_allocator->Allocate(
...
void b2ContactSolver::InitializeVelocityConstraints() {
	for (int32 i = 0; i < m_count; ++i) {
		...
		for (int32 j = 0; j < pointCount; ++j) {
			...
			vcp->velocityBias = 0.0f;
			float32 vRel = b2Dot(vc->normal,
					vB + b2Cross(wB, vcp->rB) - vA - b2Cross(wA, vcp->rA));
...
void b2ContactSolver::SolveVelocityConstraints() {
	for (int32 i = 0; i < m_count; ++i) {
		...
		for (int32 j = 0; j < pointCount; ++j) {
			...
			float32 newImpulse = b2Clamp(vcp->tangentImpulse + lambda,
...
struct b2PositionSolverManifold {
	void Initialize(b2ContactPositionConstraint* pc, const b2Transform& xfA,
			const b2Transform& xfB, int32 index) {
		...
		case b2Manifold::e_circles: {
			...
			separation = b2Dot(pointB - pointA, normal) - pc->radiusA
					- pc->radiusB;
		}
			break;
		...
		case b2Manifold::e_faceA: {
			...
			separation = b2Dot(clipPoint - planePoint, normal) - pc->radiusA
					- pc->radiusB;
			point = clipPoint;
		}
			break;
...
bool b2ContactSolver::SolvePositionConstraints() {
	...
	for (int32 i = 0; i < m_count; ++i) {
		...
		for (int32 j = 0; j < pointCount; ++j) {
			...
			float32 C = b2Clamp(b2_baumgarte * (separation + b2_linearSlop),
...
bool b2ContactSolver::SolveTOIPositionConstraints(int32 toiIndexA,
	...
	for (int32 i = 0; i < m_count; ++i) {
		...
		for (int32 j = 0; j < pointCount; ++j) {
			...
			// Prevent large corrections and allow slop.
			float32 C = b2Clamp(b2_toiBaugarte * (separation + b2_linearSlop),
					-b2_maxLinearCorrection, 0.0f);
