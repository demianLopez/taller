...
void b2PrismaticJointDef::Initialize(b2Body* bA, b2Body* bB,
		const b2Vec2& anchor, const b2Vec2& axis) {
	bodyA = bA;
	bodyB = bB;
...
void b2PrismaticJoint::InitVelocityConstraints(const b2SolverData& data) {
	...
	if (data.step.warmStarting) {
		...
		b2Vec2 P = m_impulse.x * m_perp
				+ (m_motorImpulse + m_impulse.z) * m_axis;
		float32 LA = m_impulse.x * m_s1 + m_impulse.y
				+ (m_motorImpulse + m_impulse.z) * m_a1;
		float32 LB = m_impulse.x * m_s2 + m_impulse.y
...
bool b2PrismaticJoint::SolvePositionConstraints(const b2SolverData& data) {
	...
	if (m_enableLimit) {
		...
		if (b2Abs(
			...
			C2 = b2Clamp(translation - m_lowerTranslation + b2_linearSlop,
...
b2Vec2 b2PrismaticJoint::GetReactionForce(float32 inv_dt) const {
	return inv_dt
			* (m_impulse.x * m_perp + (m_motorImpulse + m_impulse.z) * m_axis);
}
...
float32 b2PrismaticJoint::GetJointSpeed() const {
	...
	float32 speed = b2Dot(d, b2Cross(wA, axis))
			+ b2Dot(axis, vB + b2Cross(wB, rB) - vA - b2Cross(wA, rA));
	return speed;
}
