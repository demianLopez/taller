...
void b2Fixture::Create(b2BlockAllocator* allocator, b2Body* body,
	...
	m_proxies = (b2FixtureProxy*) allocator->Allocate(
			childCount * sizeof(b2FixtureProxy));
...
void b2Fixture::Synchronize(b2BroadPhase* broadPhase,
		const b2Transform& transform1, const b2Transform& transform2) {
	if (m_proxyCount == 0) {
		return;
	}
