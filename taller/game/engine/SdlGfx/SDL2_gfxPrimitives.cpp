...
typedef struct {
	...
	int oct2;
	int quad4;
	Sint16 last1x, last1y, last2x, last2y, first1x, first1y, first2x, first2y,
			tempx, tempy;
...
/*!
 ...
 \returns Returns 0 on success, -1 on failure.
 */
int pixelRGBAWeight(SDL_Renderer * renderer, Sint16 x, Sint16 y, Uint8 r,
...
int roundedRectangleColor(SDL_Renderer * renderer, Sint16 x1, Sint16 y1,
	...
	Uint8 *c = (Uint8 *) &color;
	return roundedRectangleRGBA(renderer, x1, y1, x2, y2, rad, c[0], c[1], c[2],
			c[3]);
}
...
/*!
 ...
 \returns Returns 0 on success, -1 on failure.
 */
int _aalineRGBA(SDL_Renderer * renderer, Sint16 x1, Sint16 y1, Sint16 x2,
		Sint16 y2, Uint8 r, Uint8 g, Uint8 b, Uint8 a, int draw_endpoint) {
	...
	if (dy > dx) {
		...
		while (--dy) {
			...
			result |= pixelRGBAWeight(renderer, xx0, yy0, r, g, b, a,
					255 - wgt);
...
/*!
 ...
 \returns Returns 0 on success, -1 on failure.
 */
int filledCircleColor(SDL_Renderer * renderer, Sint16 x, Sint16 y, Sint16 rad,
		Uint32 color) {
	Uint8 *c = (Uint8 *) &color;
...
int _pieRGBA(SDL_Renderer * renderer, Sint16 x, Sint16 y, Sint16 rad,
		Sint16 start, Sint16 end, Uint8 r, Uint8 g, Uint8 b, Uint8 a,
		Uint8 filled) {
	int result;
...
/*!
 ...
 \returns Returns 0 on success, -1 on failure.
 */
int polygon(SDL_Renderer * renderer, const Sint16 * vx, const Sint16 * vy,
		int n) {
	/*
	 * Draw 
	 */
...
/*!
 ...
 \returns Returns 0 on success, -1 on failure.
 */
int filledPolygonRGBAMT(SDL_Renderer * renderer, const Sint16 * vx,
	...
	if (!gfxPrimitivesPolyAllocated) {
		...
		if (gfxPrimitivesPolyAllocated < n) {
			gfxPrimitivesPolyIntsNew = (int *) realloc(gfxPrimitivesPolyInts,
					sizeof(int) * n);
	...
	for (y = miny; (y <= maxy); y++) {
		...
		for (i = 0; (i < n); i++) {
			...
			if (((y >= y1) && (y < y2))
					|| ((y == maxy) && (y > y1) && (y <= y2))) {
				gfxPrimitivesPolyInts[ints++] = ((65536 * (y - y1)) / (y2 - y1))
						* (x2 - x1) + (65536 * x1);
			}
		}
...
/*!
 ...
 \returns Returns 0 on success, -1 on failure.
 */
int _HLineTextured(SDL_Renderer *renderer, Sint16 x1, Sint16 x2, Sint16 y,
		SDL_Texture *texture, int texture_w, int texture_h, int texture_dx,
		int texture_dy) {
	Sint16 w;
	Sint16 xtmp;
	int result = 0;
	...
	if (w <= texture_w - texture_x_walker) {
		...
		while (pixels_written < w) {
			...
			dst_rect.w = source_rect.w;
			result |= (SDL_RenderCopy(renderer, texture, &source_rect,
					&dst_rect) == 0);
...
/*!
 ...
 \returns Returns 0 on success, -1 on failure.
 */
int texturedPolygonMT(SDL_Renderer *renderer, const Sint16 * vx,
		const Sint16 * vy, int n, SDL_Surface * texture, int texture_dx,
	...
	if (!gfxPrimitivesPolyAllocated) {
		...
		if (gfxPrimitivesPolyAllocated < n) {
			gfxPrimitivesPolyInts = (int *) realloc(gfxPrimitivesPolyInts,
					sizeof(int) * n);
			gfxPrimitivesPolyAllocated = n;
		}
	}
	...
	for (y = miny; (y <= maxy); y++) {
		...
		for (i = 0; (i < ints); i += 2) {
			...
			result |= _HLineTextured(renderer, xa, xb, y, textureAsTexture,
					texture->w, texture->h, texture_dx, texture_dy);
		}
	}
...
int texturedPolygon(SDL_Renderer *renderer, const Sint16 * vx,
	 ...
	 * Draw
	 */
	return (texturedPolygonMT(renderer, vx, vy, n, texture, texture_dx,
			texture_dy, NULL, NULL));
}
...
int characterRGBA(SDL_Renderer *renderer, Sint16 x, Sint16 y, char c, Uint8 r,
	...
	if (gfxPrimitivesFont[ci] == NULL) {
		 ...
		 */
		character = SDL_CreateRGBSurface(SDL_SWSURFACE, charWidth, charHeight,
		...
		gfxPrimitivesFont[ci] = SDL_CreateTextureFromSurface(renderer,
				character);
		SDL_FreeSurface(character);
...
/*!
 ...
 \returns Returns 0 on success, -1 on failure.
 */
int _bresenhamInitialize(SDL2_gfxBresenhamIterator *b, Sint16 x1, Sint16 y1,
		Sint16 x2, Sint16 y2) {
	int temp;
 ...
 */
void _murphyIteration(SDL2_gfxMurphyIterator *m, Uint8 miter, Uint16 ml1bx,
		Uint16 ml1by, Uint16 ml2bx, Uint16 ml2by, Uint16 ml1x, Uint16 ml1y,
		Uint16 ml2x, Uint16 ml2y) {
	int atemp1, atemp2;
	int ftmp1, ftmp2;
 ...
 */
void _murphyWideline(SDL2_gfxMurphyIterator *m, Sint16 x1, Sint16 y1, Sint16 x2,
		Sint16 y2, Uint8 width, Uint8 miter) {
	float offset = (float) width / 2.f;

	Sint16 temp;
	Sint16 ptx, pty, ptxx, ptxy, ml1x, ml1y, ml2x, ml2y, ml1bx, ml1by, ml2bx,
			ml2by;
	...
	for (q = 0; dd <= tk; q++) { /* outer loop, stepping perpendicular to line */
		...
		if (d0 < m->kt) { /* square move */
			...
			if (d1 < m->kt) { /* normal diagonal */
				...
				d1 += m->kd;
				if (dd > tk) {
					_murphyIteration(m, miter, ml1bx, ml1by, ml2bx, ml2by, ml1x,
							ml1y, ml2x, ml2y);
	...
	_murphyIteration(m, miter, ml1bx, ml1by, ml2bx, ml2by, ml1x, ml1y, ml2x,
			ml2y);
}
...
int thickLineColor(SDL_Renderer *renderer, Sint16 x1, Sint16 y1, Sint16 x2,
	...
	Uint8 *c = (Uint8 *) &color;
	return thickLineRGBA(renderer, x1, y1, x2, y2, width, c[0], c[1], c[2],
			c[3]);
}
...
int thickLineRGBA(SDL_Renderer *renderer, Sint16 x1, Sint16 y1, Sint16 x2,
	...
	if ((x1 == x2) && (y1 == y2)) {
		wh = width / 2;
		return boxRGBA(renderer, x1 - wh, y1 - wh, x2 + width, y2 + width, r, g,
				b, a);
	}
